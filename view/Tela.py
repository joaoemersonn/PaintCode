...
class Tela:
    def __init__(self, window, largura, altura):
        ...
        self.botaostart.definirPosicao((
            ((largura/2)-self.botaostart.rect.h), 330))
        # (350 + (self.ajuste / 2), 330))
        # (350 + (self.ajuste / 2), 450))
        self.botaoCriar.definirPosicao((
            ((largura / 2) - self.botaoCriar.rect.h), 450))
        self.botaoSair.definirPosicao((
            ((largura / 2) - self.botaoSair.rect.h), 590))
    ...
    def desenharPainelSaves(self, jogador):
        ...
        for x in jogador:
            ...
            self.savesPane.blit(self.imagemConteiner,
                                (25, (espaco * qntsaves) + 10 + self.rolagem))
            self.savesPane.blit(
                text, (55, (espaco * qntsaves) + 25 + self.rolagem))
            self.savesPane.blit(
                text2, (75, (espaco * qntsaves) + 60 + self.rolagem))
            self.savesPane.blit(
                text3, (710, (espaco * qntsaves) + 90 + self.rolagem))
            pygame.draw.rect(self.savesPane, self.corSecundaria,
                             (75, (espaco * qntsaves) + 75 + self.rolagem, 580, 30))
    ...
    def deesenharAlerta(self, textolist):
        ...
        rect=pygame.Rect((superfice.get_rect().w / 4),
                           (superfice.get_rect().h / 4), 600, 150)
        ...
        if fases is not None:
            ...
            for fase in fases:
                ...
                pygame.gfxdraw.filled_trigon(self.janela,
                                             int(self.ajuste / 2) + intx+auxX + (
                                                 espaco * qntdFase) + + 85 + int((escala * desenho.colunas) / 2),
                                             inty+aux + 50,
                                             int(self.ajuste / 2) + intx+auxX +
                                             (espaco * qntdFase) +
                                             80, 85 + inty+aux,
